...
const Navbar = () => {
  ...
  return (
    <nav
      id="site-nav"
      className={`fixed w-full z-50 top-0 right-0 left-0 transition-all duration-300 ${
        scrolled 
          ? 'bg-white/95 backdrop-blur-2xl shadow-sm h-16 border-b border-slate-200/60' 
          : 'bg-transparent h-20'
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-8 h-full">
        <div className="relative flex h-full items-center justify-between">
            ...
            <Link href="/" className="flex items-center group transition-transform duration-300 active:scale-95 gap-2">
              <div className="w-9 h-9 bg-black rounded-xl flex items-center justify-center shadow-lg shadow-indigo-200 rotate-12 group-hover:rotate-0 transition-transform">
                <span className="text-white font-bold text-xl">
                  ...
                  <h1 className={`text-2xl font-bold bg-clip-text bg-linear-to-r tracking-tighter transition-colors duration-300 ${
                    scrolled ? 'from-slate-900 to-slate-700 text-black' : 'from-white to-white/80 text-white'
                  }`}>WismarTech</h1>
          ...
          <div className="hidden lg:flex absolute left-1/2 -translate-x-1/2 items-center z-10">
            <div className={`flex items-center p-1 border rounded-full transition-all duration-300 ${
              scrolled 
                ? 'border-slate-200/80 bg-white/80 backdrop-blur-md shadow-[0_2px_8px_-2px_rgba(0,0,0,0.05)]' 
          ...
          <div className="flex gap-3 sm:gap-5 items-center ml-auto z-20">
            ...
            */}
            ...
            <button
              ...
              className={`lg:hidden flex items-center justify-center w-11 h-11 rounded-full border transition-colors active:scale-95 ${
                scrolled 
